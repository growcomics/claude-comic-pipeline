...
- **Lettering policy is now configurable** via `mandatory_rules.allow_baked_lettering` (L19 reverses L7 Case B's earlier "never bake" rule). Default is `false` — clean panels passed to `page-composer` for vector lettering. When `true`, L19's aggressive anchoring vocabulary keeps the model in photoreal CGI while baking lettering as physical 3D scene objects.
