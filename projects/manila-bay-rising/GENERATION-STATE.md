...
### Panel-attach finding (resume detail)
- bcbf138a picker search matches the model auto-title. Character sheets: Hae-won = "Korean woman character sheet"; Cel AND Dr. Santos BOTH = "Filipina woman character sheet" (ambiguous) -> must click each + verify preview (Dr Santos: glasses+lab coat+grey hair; Cel: young+white tank). Scene auto-titles are content-based (e.g. "Hotel room facing Manila...", "Manila Baywalk at golden hour", "Entertainment City recl..."); lab title NOT "laboratory"/"UP Manila" — find by sight.
- Extension intermittently times out on screenshot/batch; go one careful action at a time; timed-out batches can misclick a gallery thumbnail -> opens edit view (click Done/back to exit).
- p01-02 composed+audited (sha 31b3bb2889c5, 1:1). Resume: open picker, attach Dr Santos sheet (search "character sheet" -> click the lab-coat/glasses one, verify preview, Add to Prompt) + lab scene; clear input; OS-type /tmp/p0102.txt prompt; submit x4; download best; judge identity; bank page:p01-02.
