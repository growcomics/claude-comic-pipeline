...
- Validation against the `not-so-supra-man` bundle: dry-run correctly chose ADD (6 live cards scanned, no title clash), flagged the out-of-window release date (2026-07-30 vs visible months 2026-08/09) and fell back to today's slot with a visible note; refusal gates verified (`--execute` without approval → exit 1 before any request; `--status posted` → impossible). The single live write then ran as its own per-action act with in-session owner approval (slot=2026-08-11): card `po_1df2b9753a` created, read back live with all five chips `todo` / status draft; receipt at `projects/not-so-supra-man/posting/board-item.json`.
