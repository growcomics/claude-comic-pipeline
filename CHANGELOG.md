...
- **Verification status for the above:** `node --check` on both scripts, JSON parse of manifest + deck, and a byte-identical sync of the five files into the primary checkout's copy — the path Chrome has the unpacked extension loaded from. The live composer check (hand-typed two-paragraph prompt + two staged refs → append a card → original text and ref chips survive, no generation fired) is **owed until the extension is reloaded at `chrome://extensions`**, a click the Chrome MCP cannot deliver (`chrome://` URLs are unreachable from it). Recorded so the entry gets amended rather than assumed.
