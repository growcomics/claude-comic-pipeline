...
- **`studio/bridge.php` — key-gated AI-organizer bridge.** Lets a Claude Code session (the "AI organizer", like `comic-folder-organizer`) pull a project's uploaded drafts (base64 over HTTP, gated by a rotatable secret in `data/bridge.json`), judge them, and write ratings / best-of-group / cover back to the Studio. First run grouped Jacked Jill's 20 Higgsfield variants into 6 beats and kept the best of each. Key lives at `~/Documents/.3dmc-studio-bridge-key` (outside the repo). *Next: a full-res defect-level pass, packaging the pull→judge→push as a reusable tool/skill, and optionally a self-contained in-app "Auto-pick" button (server-side Claude API).*
