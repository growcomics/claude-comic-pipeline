...
- **Studio: download winners.** `studio/export.php` zips a project's kept images (full-res, beat order, + a `manifest.txt` mapping page → beat → original gen filename); a "⬇ Download winners (N)" button on the project header. First half of the Studio's export/port-to-comic back end (porting winners into 3dmusclecomics series — new or existing part — is next).
