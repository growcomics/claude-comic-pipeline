...
- **🎥 Angle card button in 3DMC Studio Tools (v2.7.2 → v2.8.0)** (`studio/extension/flow-studio-tools/`). The Deed Arts angle study left 76 appendable camera/pose seeds in `research/comic-corpus/synthesis/cards.json` (plus `angle-deck.md` and the HTML page's copy buttons), but none of it was reachable from the Flow panel where manual driving actually happens — a seed had to be copied out of a document and pasted by hand, which is exactly the step that gets skipped under throughput pressure. The new button sits beside 📐 Staging and opens a small picker: filter chips for the **angle** (worm / low / eye / high / dutch / OTS) and for the **muscle the shot sells** (arms / chest / back / glutes / shoulders / abs / full-silhouette), each chip counting the cards it would leave; a **🎲 Deal me one** button that appends one random card from the current filter; and the full card list (steal score, pose family, distance / angle / camera height, crop, body line, source panel — hover for the whole sentence, click to append). Selecting a card **appends** its text as the prompt's final sentence through the very same path Staging uses — `insertBlock()`, the insert half of the old `insertPromptBlock` split out so both callers share it: Slate's own `editor.insertText` via `flow-inject.js` with `where:"end"` — so it never clears the composer, never touches the staged ref chips, never submits, never selects-all (the Prompt Deck rules). Last-used chips persist in `chrome.storage.local` as `pbAngle`, read guarded: a stored value the current deck no longer knows is dropped instead of hiding every card behind a chip that isn't drawn, and values a regenerated deck introduces beyond the spec'd chips (`angle_deck.py` also knows bird / aerial / pov) get a chip automatically. The deck ships as a **bundled copy** (`cards.json`, 76 cards) served by `background.js` on an `angleDeck` message — a content script can only fetch extension files exposed to the page via `web_accessible_resources`, and the deck has no business being page-readable. Regeneration is documented in the README (`angle_deck.py --cards …` then `cp` into the extension folder; never hand-edit either copy). `manifest.json` 2.7.2 → 2.8.0.
- **Verified live on Flow (2026-09-03, growcomics, project `207a59e7`):** a hand-typed two-paragraph prompt (shift+Return breaks), two refs staged from the project's own media via *Add ingredients → Add to prompt*, then the extension's insert branch run against the composer with a real deck card (`deed-arts-omega-device-p1-1`). Result: both paragraphs intact, both ref chips intact (2 → 2), the card landed as a new final paragraph after a blank spacer paragraph, *Start generation* never touched. The picker itself — chips with counts, Low + Chest → 10 / 76, 🎲 Deal, card click, All cards, `pbAngle` persisted and restored across a reload, close / reopen — was exercised against the real `content.js` in a harness with stubbed `chrome.*`. The packaged copy still needs its **Reload** at `chrome://extensions` on the operator's machine; the Chrome MCP cannot reach `chrome://` URLs, so that click is owed to the operator.

### Fixed

- **The extension could not inject on live Flow at all — Flow moved to `flow.google.com`.** Found while verifying the button: `labs.google/fx/tools/flow/project/…` now redirects to `flow.google.com/project/…`, which matched nothing in the manifest, so no panel, no Staging button, nothing. `manifest.json` gains `https://flow.google.com/*` in `host_permissions` and in both content-script `matches` (the Slate bridge entry included). The new Flow is an **Angular** app with a **ProseMirror** composer (`flow-rich-text-editor > .ProseMirror`), not React + Slate, so `findPromptBox()` finds no Slate editor there and takes the generic-contenteditable branch — caret to the last text node, `execCommand("insertText")`, which ProseMirror's DOM observer absorbs into its document — verified as above. **Scope note:** only the prompt buttons are verified on the new domain; Download / → Studio / Review / Delete / Bake-off still harvest the `labs.google` tRPC endpoints (`flow-core.js`) and hunt for the Slate-era Create button (`flow-bakeoff.js`), and are unverified there. The README's insert-path paragraph records all of this.
...
- `research/comic-corpus/synthesis/deed-arts-staging-study.html` — the whole study rendered as one readable page (summary, camera stats, the eight moves, the 76-card deck with angle/muscle filters and copy buttons, L40, both full book analyses). Built from the markdown + JSON sources by a scratch script; regenerate when the sources change.
...
## 2026-08-10 (Publisher Wave-2 — crop renderer + first real engagement capture; landed 2026-08-11T06:55Z UTC)

**Added**
- **`skills/publisher/scripts/render_crops.py`** — executes `posting/bundle/crop-specs.json` into an actual per-platform upload kit at `posting/bundle/crops/<platform>/` (true crops + native full-res copies + `RENDER-MANIFEST.json` provenance, including per-file requested-vs-actual dims and stale-file reporting). Pillow engine with a `/usr/bin/sips` fallback — both validated on the same bundle with identical output dimensions. Center-crop v1 stands in for attention-center gravity (subject detection is a later wave); a spec larger than the source keeps native resolution at the target aspect instead of upscaling (`--allow-upscale` forces exact pixels). Local-only by construction: no network-capable imports, `subprocess` exists solely to exec the local sips binary. **Rendered binaries never enter git** — new `.gitignore` rule `projects/*/posting/bundle/crops/` covers the whole dir. Validated on `not-so-supra-man`: 10 crops + 8 copies; landscape sources hit exact spec (1200×675), portrait sources fall back to native-res 16:9/4:5 as designed; visual check clean.
- **`skills/publisher/scripts/capture_engagement.py`** — the first REAL analytics capture (the flywheel's read side per `references/analytics-capture.md`). Appends ga4 + patreon entries to `projects/<p>/analytics/engagement-stub.json`, append-only — prior captures are never modified, so deltas stay computable. Credential rule enforced structurally: it executes ONLY the path-pinned `~/Documents/.credentials/bin/{ga,patreon}` wrappers — never token files, never the Keychain, no secret ever enters context. ga4 = activeUsers summed over slug-matching top-pagePath rows, recording an **honest null + note when the comic sits below the wrapper's top-N cutoff** (below-cutoff ≠ zero). patreon = full roster paged with restricted fields (status/cents/pledge-start; aggregates only, no per-member data stored) → paying = `active_patron` AND entitled cents>0, new-patrons-since-release, entitled run-rate embedded machine-readably in notes so the NEXT pass auto-computes `patron_delta`/`revenue_delta_usd` (delta branch validated: a follow-up dry-run diffed 0/0.0 against the fresh capture). Read + local-append only; exit 2 on partial source failure so automation notices.
- **Validation capture committed** (`projects/not-so-supra-man/analytics/engagement-stub.json`, growgetter, release 2026-07-30, window +11d): patreon **471 paying / 14 new since release / $9,323.23-mo entitled run-rate** (headline patron_count 3,533 — mostly free/former, as the token memory documents); ga4 null-with-note (comic slug below the wrapper's top-8 cutoff of ~696 activeUsers). The `capture_plan` (+7d/+30d) is deliberately untouched — plan entries without matching captures remain the "pass owed" signal.

**Changed**
- `skills/publisher/SKILL.md` — workflow gains step 3 "render the crop specs" (old 3/4 renumber to 4/5); bundle table documents `crops/` as local-only/gitignored and points the analytics row at the capture script; hard rules extend to "never commit rendered crops" and name `capture_engagement.py` as the only sanctioned capture path.
- `skills/publisher/references/analytics-capture.md` — ga4+patreon flipped from "Wave-2 wires this" to WIRED, with usage, the GA top-N caveat, and the no-PII roster note; other sources stay Tier-2/manual. Cadence section now routes the ga4/patreon share of each pass through the script.
- `skills/publisher/scripts/prepare_post.py` + `references/engagement-stub-schema.json` — spec/stub prose updated to point at the renderer and capture script (crop-specs `_note`, engagement-stub `_note`, checklist step 6, schema description). No behavior change; bundles already emitted keep their old prose.

**Unchanged, on purpose: the hard rule.** Neither new script posts, uploads, or deploys — rendering is local file writes into a gitignored dir, capture is wrapper reads + a local append. `posting/posted.json` stays human-only; publishing remains a per-action human act.
...
**Added**
- **`skills/writer/`** — the WRITER stage (`docs/PRODUCTION-SYSTEM-VISION.md` §2 Stage 2, previously ❌ missing): expands one selected ideator concept (contract: `skills/ideator/references/concept-schema.json`, `selected_concept_id`) into a full panel-ready `script.md` in the exact shape `script-breakdown` consumes, pre-baking everything the pipeline is opinionated about so Stage 3 transcribes instead of inventing. Pieces:
  - `SKILL.md` — triggers ("write the script" / "expand this concept" / "script this pitch"), the poll→pre-plan→write→validate→human-gate workflow, the I/O contract named explicitly.
  - `references/script-format.md` — the Writer→Storyboard contract made explicit (vision §4's "partly implicit" handoff): header pre-answering script-breakdown's Step 0, story-spine block (forward-compatible with the in-flight L36 `story_spine`/`distinguishing_marks` gate on `feat/comic-corpus`), cast/locations/scenes declarations, PANEL grammar with `[beat/tier/camera/size/staging/chars]` annotations, canonical vocabularies, script→shotlist mapping table, worked mini example.
  - `references/craft-digest.md` — the write-to-the-pipeline's-strengths ruleset with citations: growth-density floors by chapter type (corpus F1), breasts→glutes→muscles order, escalation-device selection (≥2/scene), tier-curve declaration + never-invent-state-changes, L12 close-framed dialogue, L13 per-speaker splits, L34 staging named at script level, L35 money-shot faces, always-clothed, no-extras, per-type conventions.
  - `references/gribble-alignment.md` — what `studio/gribble.php` does (measured 41-script formula, parse→score→repair loop, gates calibrated against the real author), what the Writer reuses (the loop, the structural numbers, the grid-break translated to splash reveals) vs does differently (concept-driven story, not Gribble's signature shape; annotated format, not prose; pipeline vocabulary), and where the imitation-gate calibration doctrine applies to the Writer's own gate.
  - `templates/{fmg,be,bg,mmg}-beat-template.md` — per-transformation-type beat-sheet skeletons (types per `production-briefing`).
  - `scripts/validate_script.py` — mechanical gate (gribble.php pattern, ported to the pipeline's targets): parse into the contract shape, growth-density ratio vs chapter-type floor, L13 dialogue limits + 25-word balloons, tier-curve monotonicity/start/end conformance, scene decomposition (setup/regions/reveal), coverage + extras lint, L20/L34/L35 pre-checks of what Gate B hard-fails downstream. Verified to bite via 12 seeded-violation variants (each exits 1 with the expected code).
  - `samples/spot-me/` — the zero-credit worked example: `concept.json` (schema-valid — passes the ideator engine's own `tournament.py validate`, selected) → `script.md` (6pp/26 panels, 83% growth pages, tier 2→5, two decomposed scenes, L39 registers on every multi-character panel) → `shotlist.json`/`.md` + `references_required.json`. Gates re-run after rebasing onto the ideator-engine/L38/L39 main (`86841ac`): validate_script clean (0/0), Gate A clean (26 panels, 0 warnings), Gate B **zero HARD outside the inherently-pre-generation categories** (`asset`×26, `reference_completeness`×14 — the downstream stages' work orders) **and zero SOFT — including the live L38 `story_spine` gate**. Full record in `samples/spot-me/VALIDATION.md`.

**Changed**
- `docs/PRODUCTION-SYSTEM-VISION.md` — Stage 2 flipped **MISSING → BUILT ✓** (§2 block, §3 heat-map row, §4 Writer→Storyboard contract row now "schema exists": `script-format.md` + `validate_script.py`), per the vision doc's standing instruction.

Mid-flight contract drift, absorbed: the branch was authored against `main@feeca4d`; main then landed the ideator engine (Stage 1), the publisher prep (Stage 7), **L38** (story spine — the writer's spine block + `distinguishing_marks` now map to a live HARD gate) and **L39** (situation registers — added `[situation:]` to the format, a required-on-multi-character check to the validator, and `panel_situation` to the sample). Gate mismatches found while validating, worked around in the format doc and flagged for fix (spawned task; both verified still present on `86841ac`): `cowboy` is a legal rules_audit distance but missing from Gate A's KNOWN_VIEWS; rules_audit's L34 staging check reads `panel.cast` where the schema says `characters`, so it can never fire.
...
- **`skills/continuity-check/scripts/vision_shadow.py`** — advisory shadow sidecar for experiment 02's vision audit. Three modes: `plan` (walks banked logs — `pages-log.json` done-dicts AND the flat/`entries`-wrapped hf-log dialects — resolves local images, assembles the per-project rubric, emits subagent batch manifests), `ingest` (normalizes raw subagent verdicts into `qa/receipts/<job>.vision.json` ADVISORY receipts with canonical registry IDs per flag), `report` (agreement vs banked ground truth → `qa/vision-shadow-report.md` + `agreement.json`). **ZERO gate contact by construction**: never imports/executes project `qa/*.py`, never touches MANIFEST/ledger/pages-log; works regardless of gate lock state. Why: experiment 02's verdict was "don't ship a gate yet — labeled data too thin"; months of banked production verdicts solved the data problem, and a sidecar measures agreement without spending the integrity-lock's re-bless budget.
- **`skills/continuity-check/references/vision-shadow-rubric-core.md`** — the ship-spec rubric (v3 categories + v5 confidence semantics, verbatim, provenance-annotated) with a marked `location_mismatch` extension (ENV-01) and a per-project CAST CANON insert slot; per-project canons at `projects/{not-so-supra-man,scientists}/qa/vision-shadow/cast-canon.md`.
- **`docs/experiments/02-vision-audit-pilot/shadow-agreement-2026-08-10.md`** — cross-project rollup + promote/iterate/park decision. Headline: strictly vs banked-subagent GT no category clears the ship bar (recall ≥80% + precision ≥70%, support ≥5; WARD closest at 77%/65% on support 22) — but orchestrator adjudication confirmed every eyeballed vision-only flag as REAL (12-panel Destroya hair-drift the human audit never logged; p19/p21 wardrobe escapes; scientists p14-01 costume-state escape), while confirming real vision misses on style (p39 anime BLOCKER) and size (p30 doll-scale). Decisions: WARD + HAIR promote as always-on advisory (🏴-queue candidates), ENV/LETTER iterate, COUNT/ANATOMY/COMPOSITE advisory pending support, SIZE + STYLE + IDENT park (size stays with deterministic anchor gates).
- **Per-project shadow artifacts** — `qa/vision-shadow/` (assembled rubric, plan, batch manifests, 127 raw sonnet verdicts, notes-classification for scientists, agreement.json) + 127 advisory `qa/receipts/*.vision.json` + `qa/vision-shadow-report.md` with per-panel disagreement drill-downs and adjudication appendices. Run cost: 17 sonnet subagents, zero generation credits.
- **not-so-supra-man ground-truth text now tracked** — `qa-report.md` (the 2026-06-10 46-page human audit table this run scored against; verified same-generation as the on-disk panels) + `hf-log-{A,B,C}.json` runner logs (the banked-entry source the sidecar walks).
