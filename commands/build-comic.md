...
- **Camera distance bias — transformation panels default to MCU or closer (L20).** For transformation comics specifically: body-region beats need the region to dominate the frame. Full-body framing makes the changing region small and the panel reads as "before/after" rather than "the change happening now." Enforced by `rules_audit.py` `check_camera_distance_bias`: HARD if chapter mean camera distance > 3.0 on the cinematic-framing scale (0 ecu-face → 6 wide-establish); HARD if fewer than 30% of panels sit in middle distances (`mcu` / `medium` / `cowboy`); SOFT per-beat warning when a non-`reveal` transformation beat is shot at a distance wider than the per-beat ceiling in `script-breakdown SKILL.md` § Step 4.5. Reserve full-body for the reveal beat only. `next_panel.py` emits `WARNING_CAMERA_TOO_FAR_FOR_BEAT` at planning time. HALT on the HARD findings and on the per-panel WARNING the same way as `MISSING_*` entries. Empirical basis: hand-made April mean 2.4 vs AI-generated April 4.1, with the AI bimodal (full + ECU, zero in the middle). See `skills/comic-production/references/camera-distance-analysis/README.md` for the source data.
