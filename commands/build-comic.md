...
|---|---|---|
| 1. Script breakdown | `shotlist.json` exists at project root **AND** `validate_shotlist.py` exits 0 (schema gate: no prose-in-camera, no unknown view tokens, no non-int tier, no on-screen dialogue missing speaker) **AND** `rules_audit.py` returns no HARD findings on the shotlist (semantic gate: camera same-combo overuse, transformation-beats coverage, required metadata: `style`, `location_strategy`, `transformation_metadata` when applicable). Surface SOFT findings but don't block. | `script-breakdown` |
| 2. References | `references_required.json` exists at project root **AND** `rules_audit.py check_reference_completeness` returns no HARD findings — every file declared in the manifest is on disk. Per L28: the manifest is emitted by `script-breakdown` Step 7, walked by `reference-gathering` in manifest-driven mode, and gated here. | `reference-gathering` |
| 3. Generation | every `panel_id` in `shotlist.json` has a matching `pages/panels/<panel_id>.png` | hand off to `comic-production` — for **Flow**, use the `flow_runner.py` batch driver (`scripts/flow_runner.py`; per-panel `next`→pick→`accept` loop, x4 default, Claude picks variant) per `references/shotlist-driven-flow.md`; for **Higgsfield**, translate shotlist → `panels.json` and run `runner.py`. **Act-boundary continuity gate**: at the end of every act (see *Act boundaries* below), run the rules continuity audit. In `auto` mode pause for sign-off; in `autopilot` mode use `policies.act_boundary_audit` from `production-config.json` to decide. |
...
**Stage 3 (Generation) — runner subprocess hand-off — NEW IN v4.** When the pipeline reaches Stage 3, do NOT enter a per-panel loop in Claude Code. Instead, hand off to a deterministic Python runner subprocess that drives Flow or Higgsfield without Claude Code involvement:
   ...
   **Flow — the shipping implementation (`scripts/flow_runner.py`).** The fully-autonomous `runners/<platform>_runner.py` with in-process Claude-API picking described above is the *target* design and is not yet built. What ships today for Flow is `scripts/flow_runner.py` — a Playwright/CDP batch driver that connects to the user's already-signed-in Chrome (`--remote-debugging-port=9222`), reuses `next_panel.py --as-json` for ALL composition (view-aware chaining, ref selection, lineup/tier attachment, prompt — identical logic, nothing duplicated), and does only the UI mechanics: set aspect/count, **upload every ref via Flow's file input** (deterministic — each ref `next_panel.py` emits is a local file path, so this sidesteps the flaky gallery drag the docs warn against), type the single-line prompt, submit, wait, download `pages/panels/<panel_id>/v1..v4.png`. It is driven **per panel** by Claude Code, not as one fire-and-forget subprocess:

   1. **Preflight (once per run):** `curl http://127.0.0.1:9222/json/version` to confirm Chrome+Flow are up; if it fails, surface the launch command from `flow_runner_README.md` and halt (missing-input). Then `flow_runner.py <project> probe` once to confirm selectors resolve against the live DOM; tune `<project>/.flow/selectors.json` if any key reads `matched: false`.
   2. **Generate:** `~/.flow-venv/bin/python <skill>/scripts/flow_runner.py <project> next` → uploads refs, submits x4, downloads variants, prints `{"status":"awaiting_pick", ...}`.
   3. **Pick:** Claude Code reads the four variant PNGs and picks the best per `shotlist-driven-flow.md` step 6. **This does not violate the autopilot no-ask rule** — picking is Claude Code inspecting images and deciding internally; it never calls `AskUserQuestion` and never interrupts the user. The subprocess design above existed to guarantee that structurally; here it's preserved by discipline. (For a *fully* unattended run with no per-panel reasoning, pass `--auto-pick v1`, or set `generation.variant_picker` to a non-`claude` value once the autonomous runner lands.)
   4. **Accept + advance:** `flow_runner.py <project> accept <panel_id> v<N>` writes `pages/panels/<panel_id>/_accepted.txt` — the same marker `next_panel.py` reads to mark a panel done. Loop to step 2.

   **Resume** is via those on-disk `_accepted.txt` markers (not `state.json`): re-running picks up the first pending panel, so a crash mid-chain loses nothing. **Status mapping to the halt machinery:** flow_runner.py exit/`status` of `halt` (L12/L13) or `error`/`timeout`, or any `MISSING_*`/`WARNING_*` in the plan, are handled exactly like the approved halt conditions below — write the reason to `.autopilot-halt-reason`, delete `.autopilot-active`, stop cleanly. See `scripts/flow_runner_README.md` for setup and the full exit-code table.
...
6. **Approved halt conditions** — autopilot is permitted to stop ONLY on these. Everything else continues with config defaults.
   ...
   | Stage 1 schema-gate fail | `validate_shotlist.py` exits 1 at end of script-breakdown (prose in `camera` field, unknown view token, non-int `tier`, on-screen dialogue missing `speaker`/`character`) | Surface the validator's `ERRORS` block. User edits shotlist; re-run autopilot. Schema gate runs before rules_audit, so this halts on bug-class issues before semantic checks fire on malformed data. |
...
- **Run both gates at the end of script-breakdown.** After writing `shotlist.json`, run BOTH in order:
  1. `python skills/script-breakdown/scripts/validate_shotlist.py .` — schema gate (fast). Catches prose in the `camera` field, unknown view tokens, non-int `tier` values, and on-screen dialogue missing both `speaker` and `character`. Exit 1 means blockers; fix and re-run before gate 2.
  2. `python skills/continuity-check/scripts/rules_audit.py --project .` — semantic gate. Surface HARD findings inline. HARD findings include: missing required metadata (`style`, `location_strategy`, transformation metadata), same camera (distance × angle) combo used in more than 3 panels, and (when `transformation_scenes` is declared) missing setup beat / fewer than 3 body-region beats / missing reveal beat.

  Block stage 2 until both gates pass — re-planning the shotlist costs nothing, regenerating panels after the fact wastes the API budget. **In autopilot mode**, behavior is governed by `policies.script_breakdown_audit`: `halt-on-hard` (default) stops cleanly on HARD findings from either gate; `log-only` records the report and continues.
...
- **Bake 2D comic-style lettering with scope-bounded overlay (L19) — unconditional.** Speech bubbles, captions, and SFX text are baked directly into the generation prompt as **flat 2D comic-book overlay graphics** — clean white rounded ovals with bold black outlines for balloons, yellow rounded rectangles with black outlines for captions, flat 2D comic-display-font ALL CAPS text for SFX. The 2D scope is **explicitly bounded to the bubble/caption/SFX graphics**; the bodies, costumes, skin, hair, and environment stay photoreal CGI. `next_panel.py` `_l19_lettering_block()` auto-emits the block when the panel has dialogue/captions/SFX. The composer also emits the closing scope-bounded negation: *"Photographic CGI render on the bodies, costumes, skin, hair, environment, and lighting; NOT a 2D illustration on the bodies, NOT cartoon-shaded skin. Only the bubble / caption / SFX graphics are flat 2D comic-book overlay."* This defuses **L7 Case B**'s 2D-drift failure mode by naming the scope of the 2D style. Test render validating the vocabulary: job `607cf047-23d2-453e`, 2026-05-16. **As of 2026-05-25 the `mandatory_rules.skip_baked_lettering` opt-out is removed** — text always ships baked into the generated panel. `page-composer` no longer letters; it is layout + PDF only.
...
- **Reference completeness is mandatory (L28).** Stage 2 cannot close until every file in `references_required.json` exists on disk. The manifest is emitted by `script-breakdown` Step 7 from the shotlist (one body-tier ref per distinct `muscle_size_tier` value, one establishing ref per location, plus reverse-view refs for any location with detected shot-reverse-shot). `reference-gathering` walks the manifest in manifest-driven mode. `rules_audit.py check_reference_completeness` HARD-fails for missing files. **Body-tier refs at tier ≥ 2 MUST be generated with the muscle-size lineup PNG attached as a reference image during their own generation** — the lineup is what makes the character's identity-at-tier-N actually look like tier N. Without lineup attachment at ref-generation time, every panel that chains off that body-tier ref inherits realistic-fitness drift; per-panel lineup attachment then has to correct the character's own ref, which is the wrong direction of the workflow.
- **Multi-angle character reference packs (L16).** Every arc character (any character with `body_tiers` in the manifest) requires 5 additional view refs at the baseline tier: `view-3q-full.png`, `view-profile.png`, `view-back-full.png`, `view-low-angle-front.png`, `view-ecu-region.png`. `script-breakdown` Step 7 emits the `views[]` block in the manifest; `reference-gathering` walks it; `rules_audit.check_reference_completeness` HARD-fails for any missing view. The view refs anchor costume details that a front-only baseline doesn't show (back collar, profile body, low-angle hero shots, mid-region crops).
- **Female characters must read as beautiful (L15).** Every panel where a female cast member is the focal subject gets the glamour anchor auto-injected by `next_panel.py` `_female_beauty_anchor_line()`. The anchor names vogue-cover face quality, sculpted features, expressive eyes, magazine-cover finish — same vocabulary that worked for the Supergirl face-card re-roll. Female detection heuristic: `cast[].sex` in `{"f","female"}` or `cast[].pronoun` in `{"she","her","her/hers","she/her"}`. Default-assumes female when sex/pronoun unset (most pipeline projects are FMG-heavy). Opt out per character via `cast[].glamour_anchor: false`.
- **Known/canonical characters can't drift (L17).** IP characters (Chun Li, Lex Luthor, Supergirl, April O'Neil, etc.) get an explicit "canonical version of X" anchor in every prompt where they appear. Cast entries opt in via `cast[].canonical: true` + a `cast[].canonical_anchor` text string describing the canonical details. `next_panel.py` `_canonical_character_directive()` reads that and prepends a directive naming both the canon and a negation of generic interpretation. `reference-gathering` prefers canon-sourced reference images for face cards when the character is flagged canonical.
- **Pose anatomy coherence (L18).** Universal soft guardrail. Every panel prompt gets an anatomy-coherence line auto-injected by `next_panel.py` `_pose_anatomy_anchor()`: torso, hips, abdomen, and feet face the same direction; no impossible twists; limbs attach naturally. Always emits; no conditions. Cheap (~30 tokens) and stacks with the other mandatory rules.
- **Camera distance bias — STRENGTHENED (L20).** Transformation comics: chapter mean distance must be ≤ **2.5** (was 3.0; tighter threshold matches the hand-made April benchmark of 2.4). Non-transformation: still ≤ 3.0. **Body-region transformation beats (`chest` / `hips` / `rear` / `arms` / `abs` / `legs` / `suit_fail`) shot at `full` or wider are HARD findings** (promoted from SOFT). `next_panel.py` `_body_region_camera_directive()` prepends aggressive ECU vocabulary to body-region beats: "EXTREME CLOSE-UP filling 70%+ of frame, macro 100mm, region DOMINATES the panel, head and feet cropped OUT, NOT a full-body shot." The "DOMINATES" + "cropped OUT" language is load-bearing — without it the model defaults to wider framings.
...
- **Muscle-size lineup ref on full-body or stage-change panels (L11).** Per L11 (expanded from L5), attach the lineup ref on every full-body camera panel of the arc character AND on every stage-change panel — not just stage changes. ECU and mcu panels skip the lineup. Always include the size number in the prompt regardless. The lineup is a 3D body chart that scales TWO proportion attributes per tier: muscle mass AND breast scale (size / fullness / forward projection). Both must be matched in the prompt with parallel **CRITICAL — MUSCLE** and **CRITICAL — BREASTS** blocks (see `references/lessons-learned.md § L11` and `references/peak-body-scale.md`). Muscle-only anchoring leaves breasts to regress to the model's average-scale prior.
