...
**Identity-ref check first (L10 — refs are truth, prompts are deltas).** Before generating anything for a character, check for `references/characters/<char_id>/internet-3d-base.png` — the photoreal 3D base anchor produced by the `reference-acquisition` skill (`skills/reference-acquisition/SKILL.md`). When it exists, every face-card and body-tier generation below ATTACHES it as the identity reference and the prompt carries only deltas — framing, expression, camera, tier build, lineup scoping — never a re-description of face/hair/costume (appearance lives in the attached image). The prose path (composing identity from `cast[].wardrobe`) is the FALLBACK, used only when no base ref exists. For a brand-new character being bootstrapped from an internet source, run `reference-acquisition` first so the base ref exists before this walker generates anything.

1. **`face_card`** — if missing on disk, generate it. Use the Higgsfield MCP (`generate_image` with `nano_banana_2`) or Flow, depending on the project's `production-config.json` platform setting. Aspect 1:1. Save to the declared path.
   - **Base ref exists**: attach `internet-3d-base.png` as the identity reference. The prompt is the framing/expression delta only — e.g. "Head-and-shoulders portrait crop of the attached character, facing camera, neutral expression, soft even studio lighting; face, hair, and costume come from the attached reference exactly" — plus the render anchor. No appearance prose.
   - **No base ref** (fallback): prompt is the character's canonical face description from `cast[].wardrobe` + a neutral expression anchor.

2. **Each `body_tiers` entry**:
   - Check if the file exists. If yes, skip.
   - If `tier == 1` (baseline): with the base ref, attach it as the identity reference and prompt only the pose/framing delta (hands-at-sides front-full pose; identity from the attached reference). Fallback with no base ref: generate from the character's wardrobe description + a hands-at-sides front-full pose. Aspect 3:4. **No lineup ref attached** at tier 1 (the character's natural baseline build is the target).
   - If `tier ≥ 2`: **HARD REQUIREMENT — attach the muscle-size lineup PNG as a reference image during generation**. Use `muscle-size-lineup.png` for tiers 1–6 or `muscle-size-lineup-4-9.png` for tiers ≥ 7. With a base ref, `internet-3d-base.png` is attached ALONGSIDE the lineup — base ref carries identity, lineup carries proportions; the two never trade roles. The lineup is a PROPORTION reference ONLY (per L11 surgical scoping): the prompt must explicitly tell the model "use figure N from the lineup ONLY for (a) muscle mass and frame width AND (b) breast SIZE / FULLNESS / forward PROJECTION; do NOT borrow face, hair, costume, or pose from the lineup; identity comes from the attached base reference" (fallback with no base ref: "identity comes from the character's wardrobe description + the face card"). Breast scale is a LOAD-BEARING attribute the lineup conveys alongside muscle scale — figure 6 has visibly larger and more forward-projected breasts than figure 1 — so the body-tier ref generation must anchor both attributes, not just muscle.
   ...
   - Compose the prompt with:
     ...
     - Subject: the character's name + cartoony hyper-FMG style anchor for tier ≥ 2 (per L11). With the base ref attached, identity is a pointer, not prose: "face, hair, and costume come from the attached base reference exactly."
     - The tier-specific **muscular-build** descriptor (per L11's `_BUILD_BY_TIER` table in `rules/l11_muscular_build.py`) — a legitimate prompt DELTA in both paths (the tier is exactly what's being changed relative to the base ref)
     - Costume: with the base ref attached, NO costume prose — the base ref carries the canonical costume; state only that the garment stays in its BASELINE un-torn state. Fallback with no base ref: the character's canonical wardrobe description (the BASELINE costume, NOT the torn-up version — the tier ref is the body, not the damage state)
...
- **Never rebuild identity from prose when a base ref exists.** If `references/characters/<char_id>/internet-3d-base.png` is on disk, face-card and body-tier generation attach it as the identity reference and prompt only deltas (per L10 and `skills/reference-acquisition/SKILL.md`). Prose identity is the no-base-ref fallback, not a stylistic choice.
...
- **Provenance still applies.** Even for AI-generated refs (not gathered from external sources), write a `_provenance.md` entry: prompt used, model, timestamp, lineup attached y/n, identity base ref (`internet-3d-base.png`) attached y/n. Future audits depend on knowing how each ref was made.
...
The city-scale sibling of the real-photo SOP above: same real-photo→CGI-plate idea, but pre-built for a whole city and reusable across projects. Both conventions are indexed by `skills/location-scout/scripts/pack_index.py` into `references/locations/index.json`.
