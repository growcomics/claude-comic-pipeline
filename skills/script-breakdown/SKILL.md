...
- `body_region_focus`, `body_region_part`, `action_lines_level`, `sfx_level`, `sfx_word`: optional **L33** body-region SFX overlay overrides. By default L33 fires automatically when `transformation_beat` is one of `{arms, chest, abs, rear, legs}` and picks calibrated A2/B2 levels (or A3/B2 at tier 6+). Set `body_region_focus: true` on a panel that frames a body region without a matching transformation_beat (e.g. a flex pose) to opt into the rule. `body_region_part` ∈ `{bicep, chest, abs, glutes, quads}` selects the region label and default SFX word. `action_lines_level` and `sfx_level` ∈ `{auto, off, subtle, medium, heavy}` override the calibrated default (`auto`); use `off` to suppress one axis. `sfx_word` overrides the per-region default (FLEX for arm/abs/glute/quad, POMF for chest). See `comic-production/references/lessons-learned.md` § L33 and `references/sfx-calibration/contact-sheet.png` for the calibration matrix.
