...
- **Dialogue** stays under 25 words per balloon. Split long speeches into multiple balloons. Dialogue lines live in the shotlist `dialogue[]` as data; `next_panel.py` auto-bakes them into the render as flat 2D comic-book speech bubbles (**L19** — scope-bounded so the bodies/scene stay photoreal CGI). `page-composer` no longer letters.
- **SFX** go on the panel they sound on, not the next. SFX is data in the shotlist `sfx[]`; `next_panel.py` auto-bakes it as flat 2D comic-style ALL-CAPS lettering (**L19**), paired with the closing scope-bounded negation. Populate `sfx[]` for growth/impact panels; physical cues (fabric strain, motion blur, sweat) render in the photoreal layer alongside the SFX overlay.
- **Captions** carry narration or time-jump cues ("Three days later."). They live in `captions[]` and are baked into the render as yellow 2D caption boxes by `next_panel.py` (**L19**) — not deferred to `page-composer`.
...
- `page-composer` reads `pages[]` for **layout + PDF only**; lettering (dialogue/captions/SFX) is already **baked into each panel** by `next_panel.py`'s `_l19_lettering_block()` (per **L19**, as scope-bounded flat 2D overlay) — composition does not letter
