...
- [L19 — Bake 2D comic-style lettering, scoped explicitly to the lettering only](#l19--bake-2d-comic-style-lettering-scoped-explicitly-to-the-lettering-only)
...
L4 was originally deprecated under L7 ("we don't bake bubbles anyway"). When L19 reversed L7, L4 came back to active. Under the May 16 L19 rewrite, L4's rules are implemented inside the auto-emitted L19 lettering block: the composer reads `dialogue[].type` per entry and emits the right bubble shape (rounded oval for `balloon`, cloud with trail of dots for `thought`, dashed-outline oval for `whisper`, jagged starburst for `shout`, off-edge tail for `off-panel`), names the speaker's side of the frame, points the tail at the named speaker, and quotes the exact text. So L4 is no longer something you hand-author — populate `dialogue[]` correctly in the shotlist and the bubble shape, position, and tail attribution are emitted for you.
...
When you put comic-coded vocabulary ("speech bubble", "POW!", "comic panel") into a CGI prompt, the model drifts toward 2D illustration training data, and you get a panel that looks half-rendered, half-cartoon. L7's diagnosis was correct; the prescription evolved across three iterations. Pre-L19: defer all lettering to vector overlays (avoidance — produced sticker-on-top look). L19 introduced 2026-05-13: bake lettering as physical 3D scene objects (held CGI but produced literal-3D bubbles). **L19 rewritten 2026-05-16 (current)**: bake lettering as flat 2D comic-book overlay graphics with the 2D scope **explicitly bounded** to the bubble/caption/SFX graphics only. The bounded scope is the key — L7's failure was that comic vocab had no scope; the May 16 fix names what's 2D (bubble graphics) and what stays photoreal (bodies, costumes, environment).
...
**This is the actual reference the L11 prompt attaches.** Six 3D-rendered figures showing TWO progressively-scaled proportion attributes per tier: (1) muscle scale (visible deltoids, biceps, chest, abs, quads, frame width) and (2) breast scale (size, fullness, forward projection). It's a body chart with rendered musculature AND tier-scaled breast proportions. It is **NOT a silhouette** (a flat outline shape) and **NOT a face / hair / costume / pose reference** (all six figures share those constants — the chart conveys only the two proportion attributes). Both load-bearing distinctions are documented in the "Important" notes at the end of this section.

Transformation comics live on size escalation: she gets bigger across the issue. The AI's default is "plausible fitness model" — a real human at the gym — AND "conservative / average breast scale" regardless of muscle tier. Neither matches the genre. The genre is comic-book proportions: deltoids 2-3x normal mass, biceps with their own gravitational field, chest depth, abdominal definition carved in 3D, AND breast scale that grows in lockstep with the muscle tier (figure 6 has visibly larger, fuller, more forward-projected breasts than figure 1 in addition to larger muscles). Without aggressive prompt vocabulary that points at BOTH **muscle volume and definition** AND **breast size / fullness / forward projection** — AND an attached *muscle-size lineup* reference — the model regresses on one or both attributes, and you end up with a tier-4 panel that looks like a tier-2 fitness model OR a tier-4 body with tier-2 breasts.

The fix is three-part. First, attach the lineup ref on every panel where the body is the focal subject (full-body shots and stage-change panels). Second, tell the model — in the prompt — exactly which figure on the lineup to match, what specifically about that figure to match along BOTH proportion attributes (muscle MASS and DEFINITION: deltoid mass, bicep peak, chest depth, abdominal definition, quad mass, frame width; AND breast SIZE, FULLNESS, forward PROJECTION), and what to *not* borrow from it (face, hair, clothing, pose, expression). Third, add the explicit negations: "NOT realistic fitness, NOT athletic, NOT a fitness model at wider scale, NOT bigger muscles with conservative breasts — cartoony FMG with HEAVY 3D muscle mass AND lineup-matched breast scale."

Tier 4 is the friction zone for muscle. Tier 3 and below the model handles fine for muscle. Tier 5 and above is so visibly exaggerated that the lineup carries it naturally for muscle — *when the prompt vocabulary points at muscle volume, not at outline shape*. Breast regression, by contrast, happens at every tier without the explicit BREAST anchor — the model's average-breast prior is strong regardless of declared tier.

### Important: the "silhouette" purge (2026-05-16 morning)
...
### Important: the breast-scale anchoring (2026-05-16 afternoon, Alignment Diff #3)

Even after the silhouette purge, breast scale on generated panels still regressed to average / conservative size regardless of declared muscle tier. User-confirmed observation: *"There is a problem with the generations in that it seldom matches the breast size of the reference attached. I did a prompt where I asked it to match the breast size of the sixth person in the muscle comparison chart and the rendered output still landed with smaller breasts than the lineup figure shows."* Tested on nano_banana_2 at 1k with the explicit user prompt asking to match figure 6 — muscle landed at tier 6, breasts landed at tier 2-3.

Diagnosis: the lineup conveys TWO proportion attributes (muscle AND breast scale), but the post-silhouette-purge vocabulary called out only muscle with caps-lock framing and "do not regress" guards. Breasts were mentioned as a passing list item ("(b) the size, fullness, and shape of the breasts") buried inside a three-part list and never given the same surgical-scoping treatment. The model's average-breast prior dominated.

The fix applies the same surgical-scoping pattern that worked for muscle: a **CRITICAL — BREASTS** block in the prompt parallel to the existing **CRITICAL — MUSCLE** block, with explicit "do not regress" guards (*"do NOT default to average / conservative breast size; do NOT render tier N muscle mass with breasts shrunk to tier 2 or 3 size; breast scale is a LOAD-BEARING attribute of the lineup, not an afterthought"*), plus style-anchor mention, stage-change verbal-fallback mention, vision-rubric verification of both attributes, and retry-strategy escalation. Six pipeline-active sites updated: `rules/l11_muscular_build.py`, `references/lessons-learned.md`, `references/peak-body-scale.md`, this section, `skills/reference-gathering/SKILL.md`, `commands/build-comic.md`.
...
![Sticker overlay vs baked-in lettering — the L19 May 16 rewrite](./the-rules-explained-graphics/06-baked-lettering.png)

This is the rule that flipped, then flipped again. **L7** originally deferred all lettering — speech bubbles, captions, SFX — to vector overlays *after* the panel was generated, to avoid the 2D drift it diagnosed. That worked but produced a "CGI panel with stickers on top" look — clean but inert.

**L19 (introduced 2026-05-13)** kept L7's diagnosis (comic vocab pulls toward 2D) and put the lettering back into the render, but rendered the lettering as **physical 3D scene objects** — chrome-extruded SFX, photoreal semi-translucent floating speech panels with real ray-traced shadows. That held the CGI register on the bodies but produced literal-3D bubbles, not classic comic-book lettering.

**L19 (rewritten 2026-05-16, current)** keeps the lettering baked in but renders it as **flat 2D comic-book overlay graphics** — clean white rounded ovals with bold black outlines, comic display font ALL CAPS text, triangular tails to speakers; yellow rounded-rectangle captions with black outlines; flat 2D SFX text. The 2D scope is **explicitly bounded to the bubble/caption/SFX graphics** — the bodies, costumes, skin, hair, and environment stay photoreal CGI. That bounded scope is the key insight: L7 Case B's failure was that comic-coded vocabulary in the prompt was *ambient* (no scope, so the model applied it everywhere). The May 16 fix names the scope explicitly so the comic style is restricted to lettering only.

The current L19 has three load-bearing pieces in every prompt:
...
- **Inject the scope-bounded lettering block** (auto-emitted by `next_panel.py` when the panel has dialogue/captions/SFX): names that 2D comic styling applies ONLY to the bubble/caption/SFX graphics; reaffirms photoreal CGI for the bodies, costumes, skin, hair, environment, and lighting; describes the bubbles as flat 2D vector overlays with NO 3D extrusion, NO chrome, NO ray-traced shadows on the scene.
- **Close with scope-bounded negation**: "Photographic CGI render on the bodies, costumes, skin, hair, environment, and lighting; NOT a 2D illustration on the bodies, NOT cartoon-shaded skin. Only the bubble / caption / SFX graphics are flat 2D comic-book overlay."

All three are load-bearing. Without the opening anchor the bodies drift toward 2D; without the bounded-scope lettering block, "speech bubble" pulls the whole panel toward illustration; without the scope-bounded closing negation, the photoreal scope isn't reaffirmed and the model treats the negation as ambient.

Test render validating the May 16 vocabulary: a two-character dialogue panel (Chun-Li + Bison in a sunlit dojo) with one balloon per speaker and one yellow caption box. Both bubbles rendered as clean white ovals with bold black outlines and comic display font text; the caption rendered as a yellow rectangle with a black outline; the bodies, costumes, and dojo environment held photoreal CGI with no 2D drift. First-shot pass.
...
Even with the muscle-size lineup attached, tier-6 panels consistently under-render — the rendered body lands at tier 4-5 proportions instead of the figure-6 target. The lineup is a six-figure chart, and when the model is asked to anchor to figure 6 specifically, it averages across all six figures in the frame. The peak figure comes out diluted.

The fix is reinforcement, not replacement: keep the lineup attached (it carries tier *context* — figure 6 relative to the other five), AND additionally attach two dedicated tier-6 anatomical reference sheets. One is a full-body reference with annotated proportion stats (biceps profile, chest / thoracic detail, waist narrowness, leg musculature); the other is a close-up anatomical detail sheet (biceps anatomy, breast volume / shape, waistline metrics, full rear view + posterior musculature). The reinforcement sheets isolate tier-6 proportions as their own dedicated anchor, separate from the multi-figure averaging.
