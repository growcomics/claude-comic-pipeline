...
| `references/prompt-templates.md` | When writing panel prompts — style prefix, shot types, transformation scene templates (A–E), environment description examples, character-size language, prompt modifications. **Active for L19 (baked lettering)**: the Mandatory Rules Block's speech-bubble lines, the Action Lines and SFX section, and the Dialogue Formatting section describe how lettering bakes into the render as scope-bounded flat 2D overlay. Read the L19 banner at the top before pasting. |
...
| `references/qa-checklist.md` | After generating panels — run before handing off to `page-composer` (layout + PDF only). Covers character consistency, environment consistency, facial expressions, camera variety check, transformation-scene QA (baked-SFX quality, stage-change lineup-ref check), anatomy (incl. FMG-specific via `fmg-anatomy-guide.md`), and chained-sequence continuity (L1/L1.5/L9). **Updated for L19 (baked lettering)**: the Dialogue and Text section and the SFX check are active and audit baked-lettering quality (legibility, attribution, no garbling, scope-bounded 2D) — there is no "no-baked-lettering" check anymore. |
...
| `references/shotlist-driven-flow.md` | When a `shotlist.json` exists and the project is being generated on Flow — the **deterministic per-panel loop** that replaces hand-driven prompt typing. Covers: runtime prompt composition (from shotlist data + observed prior-panel state, NOT pre-composed batches), automatic state-anchor selection (view-aware per L1.5), ref attachment order, the Omni-UI variant strategy (one chat submit = one image; fan out via verbatim re-runs only on novel panels or weak first results — `flow-workflow.md` "Variant Strategy"), **Claude picks the candidate** (per the autonomous-production memory), per-panel checkpoints (accept/retry/modify/skip), narrate-don't-ask interaction mode, when to actually interrupt for user judgment, and end-of-run archive cleanup. **The chain advances only when a panel is accepted**, so retries are free and unlimited. Pairs with `flow-workflow.md` (UI mechanics) and `lessons-learned.md` L1.5/L5/L7/L9. |
...
7. Mandatory rules block — read which rules apply from `production-config.json` at the project root if it exists. Compose the block from rules 1–10 minus any not in `mandatory_rules.active`, plus any `mandatory_rules.extra_lines`. **L19 baked-lettering is unconditional** (as of 2026-05-25 — opt-out removed): the composer ALWAYS emits flat 2D comic-style speech bubbles, yellow caption rectangles, and 2D SFX text whenever the panel has `dialogue[]` / `captions[]` / `sfx[]`. `page-composer` no longer letters (it is layout + PDF only) — text always ships with the generated panel. Copy the resulting block verbatim into every panel prompt.
   ...
   **Why unconditional** (as of 2026-05-25): the user explicitly removed the opt-out. Two-stage workflows (generate blank panel → page-composer adds lettering later) produce pasted-on lettering that doesn't visually integrate with the photoreal scene, and create CHANGELOG/dashboard ambiguity where "comic done" can mean either "panels generated" or "lettered." Single-stage bake means every accepted panel is the final shippable image. Test render `607cf047-23d2-453e` (2026-05-16, `nano_banana_flash`) validated the vocabulary holds first-shot; subsequent production confirmed the pattern.
...
2. **Always use `.png` URLs**, never `_min.webp`. Thumbnails degrade character consistency. *(Flow equivalent: attach prior gens per `flow-workflow.md` "Reference Attachment" — Omni-UI mechanics not yet re-verified. Flow handles the asset path internally, so the png-vs-webp distinction doesn't arise.)*
