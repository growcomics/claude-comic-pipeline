...
// ---------- 🎥 Angle card deck ----------
// content.js asks for the bundled cards.json here instead of fetching it itself: a
// content script can only fetch extension files that are exposed to the page through
// web_accessible_resources, and the deck has no business being page-readable. The
// file is a COPY of research/comic-corpus/synthesis/cards.json, generated by
// research/comic-corpus/scripts/angle_deck.py — regenerate + re-copy, never hand-edit.
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (!msg || msg.type !== "angleDeck") return;
  fetch(chrome.runtime.getURL("cards.json"))
    .then((r) => { if (!r.ok) throw new Error("cards.json HTTP " + r.status); return r.json(); })
    .then((deck) => {
      if (!deck || !Array.isArray(deck.cards)) throw new Error("cards.json: expected { cards: [...] }");
