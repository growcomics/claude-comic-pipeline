...
- **Prompt buttons (v2.2.0)** — one-click insert of canonical prompt blocks into Flow's composer, for manual driving sessions: **📷 Cine+Light** *appends* the cinematic-framing + golden-hour volume-lighting master block, **🎬 Framing** *appends* the cinematic-framing block **only** (hero staging, no lighting — for pairing with a separate lighting choice, or when you'll relight afterward), and **🎨 DAZ style** *prepends* the canonical style prefix from `prompt-templates.md` (exact tested wording). Cine+Light and Framing both direct the camera, so they're for **fresh** generations — never pair either with the i2i keep-composition lock sentence. All three go through Flow's Slate editor input pipeline (with a React-native-value-setter textarea fallback) so the composer registers the text.
