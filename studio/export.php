<?php
// Download a project's winners (kept images), full-res, in beat order, as a zip.
...
usort($kept, fn($a, $b) => _bnum($a['group'] ?? '') <=> _bnum($b['group'] ?? '') ?: (($a['ts'] ?? 0) <=> ($b['ts'] ?? 0)));
if (!$kept) { http_response_code(404); exit('No winners marked yet — pick some first.'); }
...
foreach ($kept as $x) { $man .= sprintf("page-%02d\t%s\t%s\n", $i++, $x['group'] ?? '', $x['orig'] ?? ''); }
$zip->addFromString('manifest.txt', $man);
$zip->close();

$fn = preg_replace('/[^A-Za-z0-9._-]/', '-', $id) . '-winners.zip';
