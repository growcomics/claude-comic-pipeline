...
| `refactor/refs-are-truth-prompts-are-action` | **KEEP (unmerged)** | Structural restructure of the rules registry (attach/action/match/safety) from 2026-05-23; 11 weeks of rules evolution since (L34–L38, phase-1 registry) makes a wholesale merge regressive, and its validation failed (see yuna branch). **Sole holder of `skills/reference-acquisition/`** — which the repo `CLAUDE.md` still lists as a local skill; porting that skill (and any still-good structural ideas) needs a fresh assessment against today's pipeline. | `git show refactor/refs-are-truth-prompts-are-action:skills/reference-acquisition/SKILL.md`; blog draft `0464882`; core refactor `84ab980` |
